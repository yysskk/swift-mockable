...
- `resetMock()` cleared a mock's static tracking state one slot at a time. Static state is lock-backed even in a mock whose instance state is not, so each assignment took the lock separately and a caller recording a call concurrently could observe a reset call count alongside arguments that had not been cleared yet. The static slots are now cleared in a single acquisition, matching what `Sendable` and actor mocks already did for their instance state.
- Mocking of a protocol with a constrained `associatedtype` and no default. The mock fulfills an associated type with a `typealias`, using `Any` when the requirement has no default, and `Any` does not satisfy a conformance or a `where` clause — so `associatedtype Item: Decodable` produced a mock that did not conform, reported as an error naming the mock rather than the requirement behind it. The requirement is now reported where it is declared, with the suggestion to give it a default (`associatedtype Item: Decodable = Data`), which was already mocked correctly.
- A generic `typealias` declared by a protocol lost its generic parameter and `where` clauses when re-emitted on the mock, so `typealias Pair<T> = (T, T)` became `typealias Pair = (T, T)` and failed with "cannot find type 'T' in scope".
- Mocking of a requirement that returns `Void` written as something other than `Void` or `()`. `func ping() -> Swift.Void` was taken for a value-returning requirement, so an unset handler trapped with "pingHandler is not set" where the identical `func ping()` simply does nothing; a parenthesized `-> (Void)` behaved the same way. Every spelling now takes the no-return-value path, and none of them contributes a return-type suffix when overloads are disambiguated.
